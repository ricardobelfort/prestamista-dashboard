...
@Component({
  ...
  template: `
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" (click)="cancel.emit()">
      <div class="card w-full max-w-md p-6 bg-card" (click)="$event.stopPropagation()">
        <div class="flex items-center justify-between mb-4">
          ...
          <button class="btn btn-ghost btn-icon-sm" (click)="cancel.emit()">
        ...
        <div class="flex justify-end space-x-2">
          ...
          <button type="button" class="btn btn-destructive" (click)="confirm.emit()">
            {{ confirmText() }}
          </button>
        </div>
      </div>
